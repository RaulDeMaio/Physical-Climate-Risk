...
def normalize_intensity(values: pd.Series, quantile_cap: float = 0.95) -> pd.Series:
    ...
    cap = float(clean.quantile(quantile_cap, interpolation="nearest")) if clean.notna().any() else 0.0
    if cap <= 0.0:
...
def build_heatmap_frame(
    ...
    *,
    ...
    """
    Build value and normalized-intensity matrices for supply-chain heatmap rendering.
    """
    ...
    norm_long = normalize_intensity(matrix.stack(future_stack=True), quantile_cap=0.95)
